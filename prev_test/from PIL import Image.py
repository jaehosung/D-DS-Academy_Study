    ...
    for i in range(height):
        for j in range(width):
            ...
            for k in range(centroids.shape[0]):
                print("iv")
                print(image_vector[i][j])
                print("c")
                print(centroids[k])
                k_dist = distance(image_vector[i][j],centroids[k])
                distance.append(k_dist)
            cluster[height][width] = np.argmin(distances)
    return clusters
...
def recalc_centroids(image_vector, clusters, K):
    ...
    """
    for i in range(K):
        rgb = image_vector[randint(0,image_vector.shape[0]),randint(0,image_vector.shape[1])]
