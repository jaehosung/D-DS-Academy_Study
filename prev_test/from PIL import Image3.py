...
def preprocess(image_vector):
    number_bins=256
    image_histogram, bins = np.histogram(image_vector.flatten(), number_bins, normed=True)
...
def pick_seed(image_vector, K):
    ...
    """
    x = int((image_vector.shape[0]-1)/4)
    y = int((image_vector.shape[1]-1)/9)
    centroids = np.zeros((K, image_vector.shape[2]))

#     i = 0;
#     for l in range(4):
#         for m in range(8):
#             rgb = image_vector[x*l][y*m]
...
def postprocess(image_vector):
    ...
    # # use linear interpolation of cdf to find new pixel values
    # image_equalized = np.interp(image_vector.flatten(), bins[:-1], cdf)
    # image_vector = np.around(image_equalized.reshape(image_vector.shape))
    return image_vector
